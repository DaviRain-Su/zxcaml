...
- 默认 frontend wire 版本是 **`1.7`**。
- 读取器仍接受 `1.1` / `1.2` / `1.3` / `1.4` / `1.5` / `1.6` 的兼容形态。
...
- **1.7**：显式注解的参数以 `ty!` 标记发出（`(m (ty! (type-ref account_meta)))`），使显式 `(p : ty)` 注解优先于参数分类启发式；`--wire=1.6` 为兼容窗口。
