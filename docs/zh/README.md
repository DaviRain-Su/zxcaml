...
- **示例:** `examples/` 下有 99 个程序，覆盖 ADT、嵌套/guarded pattern、tuple、record、stdlib、closure、BPF smoke、account/syscall、CPI、SPL/ATA、account parser/view、hash/sysvar demo、order_book / dao_voting / vault / mutable_state_stress，以及 fixed-point quote 数学；独立的 `omlz test` 语料位于 `examples/tests/`
