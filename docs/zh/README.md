...
- P9 Developer Experience 文档入口:[`./diagnostics.md`](./diagnostics.md)
  ...
  [`./wire-compat.md`](./wire-compat.md) 说明当前 wire `1.7` 兼容窗口。
...
sexp `1.7` → 带 constant folding、DCE、inlining、escape analysis 地 lower 到
...
- **Wire 格式:** 版本 1.7(P1 为 `0.4`;P2 在 `0.5` 加用户 ADT、在 `0.6` 加嵌套/guarded pattern、在 `0.7` 加 tuple/record;P3 在 `0.8` 加 account/syscall 引用、在 `0.9` 加 CPI 类型/引用;P4/P5 让 instruction data 和 external declarations 走过 `1.0`;P8 为 mutual-recursion groups 升到 `1.1`;P9/DX2 为 source-location plumbing 升到 `1.2`;R8/R9 继续加入 typed-parameter/array surface;R10 为 `ref-make` / `ref-get` / `ref-set` 升到 `1.5`;为表达式级 `located` span 升到 `1.6`;`1.7` 加入显式参数注解标记,旧 wire 仅作为兼容窗口)
...
- **示例:** `examples/` 下有 106 个程序，覆盖 ADT、嵌套/guarded pattern、tuple、record、stdlib、closure、BPF smoke、account/syscall、CPI、SPL/ATA、account parser/view、hash/sysvar demo、order_book / dao_voting / vault / mutable_state_stress，以及 fixed-point quote 数学；独立的 `omlz test` 语料位于 `examples/tests/`
...
| P9+ | [Wire compatibility](./wire-compat.md) | 当前 wire `1.7`、历史 additive bump 和 deprecated 兼容窗口 |
