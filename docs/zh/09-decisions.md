...
### Revised 2026-04-28 —— P1 结果与可复现性

P1 已用修订后的 `.so` 链路完成上面的 endpoint：`examples/solana_hello.ml`
可以通过 `omlz build --target=bpf` 构建，经 Solana acceptance harness 部署，并以最小
return-0 entrypoint 成功调用。G13 可复现性检查也已通过：连续两次构建
`examples/solana_hello.ml` 得到的 BPF artifact 字节完全相同（`diff_exit=0`）。

这 **不** 扩大 BPF acceptance surface。first-class closure code pointer、syscall
binding、account input decoding 和更丰富的 Solana API 仍不属于 P1。
...
### Revised 2026-04-28 —— as-built arena 与 closure 边界

P1 runtime arena 是 `runtime/zig/arena.zig` 中的 caller-owned static-buffer bump
allocator：`fromStaticBuffer`、带对齐和 checked arithmetic 的 `alloc`、以及 `reset`。
它不拥有内存，也没有 free list 或 per-object lifetime。

P1 中 recursion lower 成两种实际形状：顶层和不逃逸的递归 helper 使用直接的
arena-threaded function；逃逸的一等 closure 在 interpreter/native 路径上使用 arena 分配的
closure record。BPF 上逃逸 closure code pointer 不是 P1 acceptance 保证；mission 在该形状上
观察到 `Relocations found but no .rodata section` linker 失败，所以 P2/P3 必须选择
直接调用 lowering、linker 支持的 rodata anchor，或通过 ADR 限制 BPF 子集。
...
### Revised 2026-04-28 —— P1 harness 与 BPF 字节检查

P1 把该不变量变成了可执行检查：`zig build test` 会在 examples/UI corpus 上运行
determinism property，比较解释器输出与 hosted Zig native 后端输出。算术语义被钉为
signed 64-bit：`+`、`-`、`*` wrap，除法/取余 truncating，除零使用稳定的
`ZXCAML_PANIC:division_by_zero` marker。

BPF 单独检查：语义 acceptance 走 Solana harness；G13 字节可复现性已在 2026-04-28 对
`examples/solana_hello.ml` 记录为 PASS。更丰富程序的 BPF value-level equivalence 属于未来工作。
...
### Revised 2026-04-28 —— 操作性 warning 噪音与 objdump 路径

P1 worker 多次观察到 macOS 上的 `sbpf-linker 0.1.8` 会打印大量
`unable to open LLVM shared lib ... .a: dlopen failed`，但最终仍成功链接。这是 LLVM proxy
在探测 archive 时产生的良性 warning；只要最终 exit code 为 0 且 `.so` 有效，CI 不应把它视为失败。

Homebrew macOS 上手动检查还可能需要 LLVM 工具完整路径，例如
`/opt/homebrew/opt/llvm@20/bin/llvm-objdump`，因为 `llvm-objdump` 不一定在 `PATH` 上。
...
### Revised 2026-04-28 —— P1 acceptance 在 v2 上验证

ADR 标题和原正文保留为历史记录。P1 的权威状态是 2026-04-27 修订加上已完成的验收证据：
`omlz build --target=bpf` 使用 `sbpf-linker --cpu v2 --export entrypoint`，Solana harness
验证的是这个 v2 artifact。`--cpu v3` 在 P1 仍是 opt-in、未验证路径；没有新的 ADR addendum
和 acceptance run 前，不得成为默认值。
