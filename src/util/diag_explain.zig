...
pub const entries = [_]Entry{
    ...
    .{
        .code = "E0100",
        .title = "Cannot resolve `open` to a file",
        .description = "ADR-016 resolves a top-level `open Foo` to `foo.ml` in the entry file's directory, and that file does not exist. Only simple module names resolve to files; the bundled stdlib is auto-opened and never resolved this way.",
        .fix = "Create the missing file next to the entry file, fix the module name spelling, or remove the `open`.",
        .hint = "ADR-016 resolves `open Foo` to `foo.ml` in the entry file's directory.",
    },
    .{
        .code = "E0101",
        .title = "`open` dependency cycle",
        .description = "The multi-file closure resolved by ADR-016 contains a cycle: following top-level `open` items leads back to a file already on the resolution stack. The diagnostic message lists the cycle path. OCaml compilation units cannot depend on each other cyclically.",
        .fix = "Break the cycle by moving the shared definitions into a third file that both sides open.",
        .hint = "Break the cycle by moving the shared definitions into a third file both sides open.",
    },
    .{
        .code = "E0102",
        .title = "Duplicate top-level name across the file closure",
        .description = "ADR-016 joins every file in the `open` closure into one flat namespace, and two files declare the same top-level value, type, or constructor name. Names are emitted unqualified, so a cross-file duplicate would be ambiguous downstream.",
        .fix = "Rename one of the colliding top-level bindings so every name in the closure is unique.",
        .hint = "ADR-016 joins the file closure into one flat namespace; rename one of the bindings.",
    },
    .{
        .code = "E0103",
        .title = "`open` of a bundled stdlib module",
        .description = "The opened module name belongs to the bundled stdlib surface (`Core`, `Generators`, or a nested module such as `Account`, `Fixed`, or `Pubkey`). These are auto-opened or matched by qualified name, so `open` would shadow the whitelist matching; user files may not take these names either.",
        .fix = "Drop the `open` and use qualified access (e.g. `Fixed.mul`), or rename your file so it does not shadow a stdlib module.",
        .hint = "The bundled stdlib is auto-opened; use qualified access (e.g. `Fixed.mul`) instead of `open`.",
    },
    .{
        .code = "E0200",
