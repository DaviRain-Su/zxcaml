...
CASES: list[tuple[str, str, str]] = [
    (
        ...
        "(zxcaml-cir 1.1 (module (type_decl (name color) (params) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (type_decl (name option) (params 'a) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (type_decl (name tree) (params 'a) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (type_decl (name tree) (params) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (type_decl (name either) (params 'a 'b) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (let entrypoint (lambda (s) "
        "(match (var s) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (let entrypoint (lambda (c) "
        "(match (var c) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (type_decl (name t) (params) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (let entrypoint (lambda (pair) "
        "(match (var pair) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (let make_op "
        "(lambda (a b) (lambda (x) "
    ...
    (
        "stdlib Crypto.blake3 uses bundled fixed hash binding",
        'let entrypoint input = Crypto.blake3 input\n',
        '(zxcaml-cir 1.1 (module (external (name "Crypto.blake3") '
        '(type (arrow bytes bytes)) (symbol "sol_blake3_alloc")) '
        "(let entrypoint (lambda (input) "
    ...
    (
        ...
        "(zxcaml-cir 1.1 (module (let entrypoint (lambda (seeds program_id) "
    ...
    (
        "top-level mutual recursion sexp - even odd",
        (
            "let rec is_even n = if n = 0 then true else is_odd (n - 1)\n"
            "and is_odd n = if n = 0 then false else is_even (n - 1)\n"
            "let entrypoint _ = if is_even 4 then 1 else 0\n"
        ),
        "(zxcaml-cir 1.1 (module "
        "(Let_rec_group (bindings "
        "(binding (name is_even) (params n) "
        "(body (if (prim \"=\" (var n) (const-int 0)) (ctor true) "
        "(app (var is_odd) (prim - (var n) (const-int 1)))))) "
        "(binding (name is_odd) (params n) "
        "(body (if (prim \"=\" (var n) (const-int 0)) (ctor false) "
        "(app (var is_even) (prim - (var n) (const-int 1)))))))) "
        "(let entrypoint (lambda (_) "
        "(if (app (var is_even) (const-int 4)) (const-int 1) (const-int 0))))))\n",
    ),
    (
        "expression-level mutual recursion sexp - nested even odd",
        (
            "let entrypoint _ =\n"
            "  let rec is_even n = if n = 0 then true else is_odd (n - 1)\n"
            "  and is_odd n = if n = 0 then false else is_even (n - 1)\n"
            "  in if is_even 7 then 1 else 0\n"
        ),
        "(zxcaml-cir 1.1 (module (let entrypoint (lambda (_) "
        "(Let_rec_group (bindings "
        "(binding (name is_even) (params n) "
        ...
        "(body (if (prim \"=\" (var n) (const-int 0)) (ctor false) "
        "(app (var is_even) (prim - (var n) (const-int 1))))))) "
        "(if (app (var is_even) (const-int 7)) (const-int 1) (const-int 0)))))))\n",
    ),
    (
        "top-level mutual recursion sexp - three functions",
        (
            "let rec f n = if n <= 0 then 0 else 1 + g (n - 1)\n"
            "and g n = if n <= 0 then 0 else 1 + h (n - 1)\n"
            "and h n = if n <= 0 then 0 else 1 + f (n - 1)\n"
            "let entrypoint _ = f 9\n"
        ),
        "(zxcaml-cir 1.1 (module "
        "(Let_rec_group (bindings "
        "(binding (name f) (params n) "
        "(body (if (prim \"<=\" (var n) (const-int 0)) (const-int 0) "
        "(prim \"+\" (const-int 1) (app (var g) (prim - (var n) (const-int 1))))))) "
        "(binding (name g) (params n) "
        "(body (if (prim \"<=\" (var n) (const-int 0)) (const-int 0) "
        "(prim \"+\" (const-int 1) (app (var h) (prim - (var n) (const-int 1))))))) "
        "(binding (name h) (params n) "
        "(body (if (prim \"<=\" (var n) (const-int 0)) (const-int 0) "
