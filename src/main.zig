...
const InputSubcommandArgs = cli.InputSubcommandArgs;
    ...
    if (command == cli.CommandKind.doctor) {
        ...
        switch (result) {
            .success => |parsed| {
                ...
                try runModule(init, parsed.module);
            },
            .failed => |code| std.process.exit(if (code == 0) 1 else code),
        }
        return;
        ...
        switch (result) {
            .success => |parsed| {
                ...
                try emitIdl(init, parsed.module, idl_args.input_file);
            },
            .failed => |code| std.process.exit(if (code == 0) 1 else code),
        }
        return;
...
fn validateCutoverApiUsageOrReport(
    allocator: std.mem.Allocator,
    io: Io,
...
fn legacyExternalReplacement(symbol: []const u8) ?[]const u8 {
    if (std.mem.eql(u8, symbol, "sol_log_")) return "Syscall.sol_log";
    if (std.mem.eql(u8, symbol, "sol_log_64_")) return "Syscall.sol_log_64";
    if (std.mem.eql(u8, symbol, "sol_log_pubkey")) return "Syscall.sol_log_pubkey";
    if (std.mem.eql(u8, symbol, "sol_sha256") or std.mem.eql(u8, symbol, "sol_sha256_alloc")) return "Crypto.sha256";
    if (std.mem.eql(u8, symbol, "sol_keccak256") or std.mem.eql(u8, symbol, "sol_keccak256_alloc")) return "Crypto.keccak256";
    if (std.mem.eql(u8, symbol, "sol_blake3") or std.mem.eql(u8, symbol, "sol_blake3_alloc")) return "Crypto.blake3";
    if (std.mem.eql(u8, symbol, "sol_secp256k1_recover_alloc")) return "Crypto.secp256k1_recover";
    if (std.mem.eql(u8, symbol, "sol_get_clock_sysvar")) return "Syscall.sol_get_clock_sysvar";
    if (std.mem.eql(u8, symbol, "sol_get_rent_lamports_per_byte_year")) return "Syscall.sol_get_rent_lamports_per_byte_year";
    if (std.mem.eql(u8, symbol, "sol_log_compute_units_")) return "Syscall.sol_log_compute_units";
    if (std.mem.eql(u8, symbol, "sol_remaining_compute_units")) return "Syscall.sol_remaining_compute_units";
    if (std.mem.eql(u8, symbol, "sysvar.readClock")) return "Sysvar.clock_from_account";
    if (std.mem.eql(u8, symbol, "sysvar.readRent")) return "Sysvar.rent_from_account";
    if (std.mem.eql(u8, symbol, "sysvar.readInstructionsHeader")) return "Sysvar.instructions_header_from_account";
    if (std.mem.eql(u8, symbol, "sysvar.readInstructionAt")) return "Sysvar.instruction_at";
    if (std.mem.eql(u8, symbol, "sysvar.readStakeHistory")) return "Sysvar.stake_history_latest_from_account";
    if (std.mem.eql(u8, symbol, "sysvar.readEpochSchedule")) return "Sysvar.epoch_schedule_from_account";
    if (std.mem.eql(u8, symbol, "Cpi.set_return_data")) return "Cpi.set_return_data";
    if (std.mem.eql(u8, symbol, "Cpi.get_return_data")) return "Cpi.get_return_data";
    if (std.mem.eql(u8, symbol, "Cpi.get_return_program_id")) return "Cpi.get_return_program_id";
    return null;
}
