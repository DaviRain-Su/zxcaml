...
| CR-14 | codegen | `src/backend/zig_codegen` (option-of-tuple layout) | Option-of-tuple values crossing function boundaries fail native/BPF codegen with an anonymous-struct type identity mismatch (`expected 'omlz_user_pick__struct_1030', found '...1013'`); the interpreter handles them fine. | Unify the synthesized struct type identity for option-of-tuple payloads across function signatures and call sites. | medium | open |
| CR-15 | core | `stdlib/core.ml` (`try_find_program_address`), `src/core/anf/call_lowering.zig` | `try_find_program_address` is declared in stdlib/core.ml but unwired in lowering, so any use fails with a bare `UnboundVariable` (no span). Blocked on CR-14 because its return type is `(bytes * int) option`. | Wire the lowering signature plus codegen/interpreter stubs once CR-14 lands; until then keep it out of the `Pda` namespace so completion does not advertise it. | medium | open |
