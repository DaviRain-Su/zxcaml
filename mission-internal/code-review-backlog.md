...
| CR-11 | lsp | `src/lsp/lsp_main.zig:616,1357,1441` | The LSP server spawns `omlz` via cwd-relative `zig-out/bin/omlz`, so an installed `omlz-lsp` only works from the repo root (same class as CR-1). | Resolve `omlz` next to the `omlz-lsp` executable, then fall back to the current default. | medium | open |
