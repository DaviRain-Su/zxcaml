...
| CR-15 | core | `stdlib/core.ml` (`try_find_program_address`), `src/core/anf/call_lowering.zig` | `try_find_program_address` is declared in stdlib/core.ml but unwired in lowering, so any use fails with a bare `UnboundVariable` (no span). Unblocked 2026-06-11 (CR-14 landed); ready to wire — its `(bytes * int) option` return type now codegens correctly. | Wire the lowering signature plus codegen/interpreter stubs; until then keep it out of the `Pda` namespace so completion does not advertise it. | medium | open |
