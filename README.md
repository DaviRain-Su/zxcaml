...
- P9 Developer Experience docs: [`docs/diagnostics.md`](./docs/diagnostics.md)
  ...
  and [`docs/wire-compat.md`](./docs/wire-compat.md) for current wire `1.7`
  compatibility.
...
`compiler-libs` → emit sexp `1.7` → lower through Core IR with constant folding, DCE, inlining, escape
...
- **Wire format:** version 1.7 (P1 `0.4`; P2 added user ADTs in `0.5`, nested/guarded patterns in `0.6`, and tuples/records in `0.7`; P3 added account/syscall references in `0.8` and CPI types/references in `0.9`; P4/P5 moved the wire through `1.0` for instruction data and external declarations; P8 moved to `1.1` for mutual-recursion groups; P9/DX2 moved to `1.2` for source-location plumbing; R8/R9 moved through typed-parameter/array surfaces; R10 moved to `1.5` for `ref-make` / `ref-get` / `ref-set`; the wire moved to `1.6` for expression-level `located` spans; `1.7` adds annotated-parameter markers while older shapes remain compatibility-only)
...
- **Mollusk SVM tests:** 45 Rust integration-test files (68 Rust test cases) in `tests/` using Mollusk SVM v0.12.1 (hello, demo, simple_cpi, counter, vault, external_demo, crypto_demo, hackathon_greet, mutable_state_stress, account_guard, real-world zignocchio ports, and SPL Token primitive coverage). `tests/bpf_test_support.rs` centralizes build/load helpers for these artifacts; the historical ELF post-pass has been removed (see `mission-internal/elf-patch-investigation.md`).
...
| P9+ | [Wire compatibility](./docs/wire-compat.md) | Current wire `1.7`, prior additive bumps, and deprecated compatibility windows |
