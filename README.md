...
- **IDL:** `omlz idl <file>` emits Anchor 0.30+ compatible JSON with SHA-256 discriminators, instruction accounts/args, account types, events, errors, and constants; `Account.is_signer` / `Account.is_writable` guard helpers feed signer/writable metadata and `error_` constants feed the IDL error table
