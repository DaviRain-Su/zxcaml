//! Codegen golden for the stdlib `Crypto.blake3` binding.
//!
//! RESPONSIBILITIES:
//! - Compile a tiny OCaml source that calls `Crypto.blake3`.
//! - Keep a focused generated-Zig snapshot for the fixed-size BLAKE3 syscall call site.
...
fn appendBlake3Lines(out: *std.ArrayList(u8), allocator: Allocator, source: []const u8) !void {
    var lines = std.mem.splitScalar(u8, source, '\n');
    while (lines.next()) |line| {
        const trimmed = std.mem.trim(u8, line, " \t\r");
        if (std.mem.indexOf(u8, trimmed, "sol_blake3") != null or
...
test "blake3 golden: Crypto.blake3 lowers to fixed 32-byte syscall wrapper" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;

    const output = "/tmp/zxcaml_blake3_codegen_bin";
    const build_argv = [_][]const u8{
        codegen_options.omlz_bin,
        "build",
        "--target=native",
        "tests/golden/blake3/crypto_blake3.ml",
        "-o",
        output,
        "--keep-zig",
    };
    ...
    if (build.exit_code != 0) {
        std.debug.print("blake3 codegen build failed\nstdout:\n{s}\nstderr:\n{s}\n", .{ build.stdout, build.stderr });
    }
    ...
    try appendBlake3Lines(&actual, allocator, source);

    const expected = try std.Io.Dir.cwd().readFileAlloc(
        io,
        "tests/golden/blake3/crypto_blake3.zig.snapshot",
        allocator,
        .limited(4096),
    );
