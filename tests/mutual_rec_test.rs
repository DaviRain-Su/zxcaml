...
fn compile_program() -> CompiledProgram {
    ...
    let output_path = root.join("build").join("mutual_rec.so");

    let mut command = Command::new(root.join("zig-out").join("bin").join("omlz"));
    command.current_dir(&root).args([
        "build",
        "--target=bpf",
        "examples/mutual_rec.ml",
        "-o",
        "build/mutual_rec.so",
    ]);
    ...
    let result = command.output().unwrap_or_else(|error| {
        panic!("failed to spawn `zig-out/bin/omlz build --target=bpf examples/mutual_rec.ml -o build/mutual_rec.so`: {error}")
    });
    assert!(
        result.status.success(),
        "`zig-out/bin/omlz build --target=bpf examples/mutual_rec.ml -o build/mutual_rec.so` failed\nstdout:\n{}\nstderr:\n{}",
