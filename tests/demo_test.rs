//! SVM-level integration test for the ZxCaml demo program.
//!
//! The setup step compiles `examples/demo.ml` to `build/demo.so` with the
...
fn setup_mollusk() -> Mollusk {
    let elf_path = bpf_test_support::compile_program("demo");
...
#[test]
fn test_demo_executes_successfully_and_logs() {
    ...
    assert!(
        messages
            .iter()
            .any(|message| message.contains("ZxCaml demo")),
        "demo should emit its string log; captured logs: {messages:?}"
    );
    assert!(
        messages.len() >= 2,
        "demo should emit both string and numeric logs; captured logs: {messages:?}"
    );
}
